...
<nav aria-label="...">
    <ul class="pagination">
        ...
        <li class="page-item {{ $pedidos->previousPageUrl() ? '' : 'disabled' }}">
            <a class="page-link" href="{{ $pedidos->previousPageUrl() ?: '#' }}">Previous</a>
        </li>

        {{-- Páginas numéricas --}}
        @for ($i = 1; $i <= $pedidos->lastPage(); $i++)
            <li class="page-item {{ $i == $pedidos->currentPage() ? 'active' : '' }}">
        ...
        <li class="page-item {{ $pedidos->nextPageUrl() ? '' : 'disabled' }}">
            <a class="page-link" href="{{ $pedidos->nextPageUrl() ?: '#' }}">Next</a>
        </li>
    </ul>
</nav>
